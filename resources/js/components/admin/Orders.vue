                                ...
                                <div class="card-body">
                                    <div>
                                        <div class=" table-responsive" id="printableTable">
                                            ...
                                            <select name="sale_length" aria-controls="sale" class="" v-model="limit" v-on:change="fetchorders()">
                                            ...
                                            <div id="sale_filter" class="dataTables_filter"><label>Search:<input type="search" class="" placeholder="" aria-controls="sale" v-model="searchParameter" @keyup="fetchorders()"></label></div>
                                                <table id="sale" class="display dataTable no-footer" sale="grid">
                                                    ...
                                                    <tbody class="kt-table-tbody text-dark">
                                                        <tr class="kt-table-row kt-table-row-level-0 odd" sale="row" v-for="order in orders" v-bind:key="order.id">
                                                            <td>
                                                                {{ order.order_id }}
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td>
                                                                <a  :href="'/admin/order/'+order.order_id" class=" click-edit" id="click-edit1" data-toggle="tooltip" title="" data-placement="right" data-original-title="Check out more demos" ><i class="fa fa-edit"></i></a>
                                                ...
                                                <ul class="pagination pagination-sm m-0 float-right">
                                                    <li v-bind:class="[{disabled: !pagination.prev_page_url}]"><a class="page-link" href="#" @click="fetchorders(pagination.prev_page_url)">Previous</a></li>

                                                    <li class="disabled"><a class="page-link text-dark" href="#">Page {{ pagination.current_page }} of {{ pagination.last_page }}</a></li>

                                                    <li v-bind:class="[{disabled: !pagination.next_page_url}]" class="page-item"><a class="page-link" href="#" @click="deleteOrders(pagination.next_page_url)">Next</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
    ...
    methods: {
        ...
        deleteOrders(id) {
            if (confirm('Are You Sure?')) {
                this.$parent.loading = true;
                axios.delete(`/api/admin/order/${id}`,this.token)
                    .then(res => {
